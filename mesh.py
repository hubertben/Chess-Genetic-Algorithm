...
class Net:
    ...
    def merge(self, partner):
        ...
        child = Net(self.layer_sizes)
        child.createWeightLayers()
        
        for c, s, p in zip(child.weighted_layers, self.weighted_layers, partner.weighted_layers):
         
            for i in range(len(s.weights)):

                val = s.weights[i].value if random.random() > .5 else p.weights[i].value 
                c.weights[i].setValue(val) 
